...
- (void)viewDidLayoutSubviews
{
  ...
  ;
    ...
    self.titleScrollView.frame = CGRectMake(0, IS_X?NAVI_SUBVIEW_Y_iphoneX:NAVI_SUBVIEW_Y_Normal, ScreenW, tH);
    self.contentScrollView.frame = CGRectMake(0,tY , ScreenW, ScreenH - tY);
}
...
- (void)setUpAllTitle
{
    ...
    for (NSInteger i = 0; i < VCCount; i++) {
        ...
        CGFloat framex = buttonX ;
        CGFloat frameWidth = buttonW ;
        CGFloat frameY = buttonH - (progressH + 2);
    ...
    if (_isShowPregressView && !_pregressView) { //如果没子控制器的时候不加载
        ...
        _pregressView.frame = CGRectMake(0, buttonH -0.5, VCCount * buttonW, 0.5);
