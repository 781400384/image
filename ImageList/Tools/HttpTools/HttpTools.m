...
+(void)getWithBasePath:(NSString *)basePath  Path:(NSString *)path params:(NSDictionary *)params success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure{
    //判断网络状态
 
    
    if (![self requestBeforeJudgeConnect]) {
//        NSError *error = [[NSError alloc] initWithDomain:@"com.alamofire.serialization.response.error.string" code:NETWORK_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"网络不可用" forKey:ERROR_KEY]];
//        failure(error);
    ...
    //获取完整的url路径
    NSString * url = [basePath stringByAppendingPathComponent:path];
    
    
    
//    [[AFHttpClient sharedClient].requestSerializer setValue:[UserInfoDefaults
//                                                             userInfo]._token forHTTPHeaderField:@"x-token"];
...
+ (void)getWithBasePath:(NSString *)basePath  Path:(NSString *)path params:(NSDictionary *)params responseCache:(HttpResponseCacheBlock)cache success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure {
    
    NSString *url = [basePath stringByAppendingString:path];
    ...
    //判断网络状态
    if (![self requestBeforeJudgeConnect]) {
//        NSError *error = [[NSError alloc] initWithDomain:@"netWorkError" code:NETWORK_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"网络不可用" forKey:ERROR_KEY]];
//        failure(error);
...
+ (void)postWithBasePath:(NSString *)basePath Path:(NSString *)path
    ...
    //判断网络状态
    if (![self requestBeforeJudgeConnect]) {
//        NSError *error = [[NSError alloc] initWithDomain:@"com.alamofire.serialization.response.error.string" code:NETWORK_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:self forKey:ERROR_KEY]];
//
//
//
//        failure(error);
...
+ (void)postWithBasePath:(NSString *)basePath Path:(NSString *)path judgeKey:(NSString *)judgeKey params:(NSDictionary *)params page:(int)page responseCache:(HttpResponseCacheBlock)cache success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure {
...
+ (void)downloadWithBasePath:(NSString *)basePath Path:(NSString *)path
                 success:(HttpSuccessBlock)success
                 failure:(HttpFailureBlock)failure
                progress:(HttpDownloadProgressBlock)progress {
    
    //获取完整的url路径
    NSString * urlString = [basePath stringByAppendingPathComponent:path];
...
+ (void)uploadImageWithBasePath:(NSString *)basePath Path:(NSString *)path
                     params:(NSDictionary *)params
...
+(void) uploadWithBasePath:(NSString *)basePath Path:(NSString *)path params:(NSDictionary *)params thumbName:(NSString *)imagekey images:(NSArray *)images success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure progress:(HttpUploadProgressBlock)progress{
    //获取完整的url路径（多张图片上传）
    NSString * urlString = [basePath stringByAppendingPathComponent:path];
